...
const App = {
  ...
  renderNavLinks() {
    ...
    const links = {
      ...
      restaurant_admin: [
        { id: "restaurant-dashboard", label: "📊 Dashboard" },
        { id: "manage-menu", label: "🍴 Manage Menu" },
        { id: "restaurant-orders", label: "📋 Orders" },
      ],
      rider: [{ id: "rider-dashboard", label: "🏍️ My Deliveries" }],
      super_admin: [
        { id: "admin-dashboard", label: "📊 Dashboard" },
        { id: "admin-orders", label: "📋 All Orders" },
        { id: "admin-users", label: "👥 Users" },
        { id: "manage-menu", label: "🍴 Menu" },
      ],
    };
    nav.innerHTML = (links[role] || [])
      .map(
        (l) =>
...
function renderOrderTable(
  orders,
  { showRiderAssign = false, showStatusSelect = false } = {},
) {
  if (orders.length === 0)
...
const Pages = {
  ...
  menu: {
    ...
    renderFilters() {
      ...
      document.getElementById("menu-filters").innerHTML = cats
        .map(
          (c) =>
    ...
    renderItems(category, btn) {
      ...
      if (!items.length) {
        grid.innerHTML = `<div class="empty-state"><div class="emoji">🍽️</div><h3>No items here</h3></div>`;
        return;
      }
    ...
    async load() {
      try {
        ...
        document.getElementById("rd-stats").innerHTML = [
          ...
          .map(
            (s) =>
              `<div class="stat-card"><div class="stat-icon">${s.icon}</div><div class="stat-label">${s.label}</div><div class="stat-value">${s.value}</div></div>`,
          )
          .join("");

        document.getElementById("rd-recent-orders").innerHTML =
          renderOrderTable(orders.slice(0, 20), {
            showStatusSelect: true,
            showRiderAssign: true,
          });
      } catch (err) {
  ...
  "restaurant-orders": {
    async load() {
      try {
        const res = await Orders.getAll();
        document.getElementById("restaurant-orders-list").innerHTML =
  ...
  "manage-menu": {
    async load() {
      try {
        ...
        if (!res.data.length) {
          grid.innerHTML = `<div class="empty-state"><div class="emoji">🍽️</div><h3>No items yet — add one!</h3></div>`;
          return;
        }
    ...
    async load() {
      try {
        ...
        document.getElementById("admin-recent-orders").innerHTML =
          renderOrderTable(orders.slice(0, 15), { showStatusSelect: true });
      } catch (err) {
...
function openCheckoutModal() {
  ...
  const items = Cart.get();
  const subtotal = Cart.total();
  document.getElementById("checkout-items-summary").innerHTML = items
    .map(
      (i) =>
        `<div style="display:flex;justify-content:space-between;font-size:14px;padding:4px 0"><span>${i.emoji || "🍽️"} ${i.name} ×${i.quantity}</span><span>₦${(i.price * i.quantity).toLocaleString()}</span></div>`,
    )
    .join("");
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    try {
      ...
      e.target.reset();
      toast(
        `Order ${data.data.orderNumber} placed! 🎉 You'll receive a confirmation email.`,
        "success",
      );
...
async function openAssignRiderModal(orderId, orderNumber) {
  try {
    ...
    const options = res.data
      .map(
        (r) =>
          `<option value="${r._id}">${r.name}${r.phone ? " · " + r.phone : ""}</option>`,
      )
      .join("");
